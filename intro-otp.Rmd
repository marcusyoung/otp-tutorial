...
To complete the tutorial you will need access to QGIS, RStudio and Firefox or Chrome web browser. The tutorial containes several ![](images/challenge.png) challenges, along with ![](images/hint.png) tips and ![](images/info.png) links to extra resources. It should take about two hours to complete.
...
Once you have extracted this zip file, you should have the folders and files listed below. **Note**: the GTFS ZIP fies must not be extracted.
...
`router-config.json` is acually read when the OTP server is started (in the next section), but we add it now for convenience.
...
airport_current <- content(airport_current, as = "text", encoding = "UTF-8") # convert to text
...
```{r head lsao, eval=TRUE, echo=TRUE}

head(gm_lsoa_centroids)

```

In addition to LSAO centroid coordinates in Eastings and Northings (British National Grid), they are also provided as latitude/longitude, in the format required by OTP.
