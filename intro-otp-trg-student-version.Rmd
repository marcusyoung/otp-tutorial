...
OTP runs within a Java virtual machine (JVM), which on the University computers is provided by the Open Java Development Kit (OpenJDK) installed. We first need to confirm that the location of OpenJDK is configured as an Environment Variable, enabling you to run the command `java` from the command prompt. Test this by entering `java -version` at the command prompt. If the version of OpenJDK is not returned, run the following command to set up the path to OpenJDK and then check again:

```{r, engine = 'bash', eval = FALSE}
set path=%path%;C:\Program Files\Amazon Corretto\jdk11.0.4_10\bin
```

![](images/hint.png) *If using your own computer, you may have the Oracle Java Runtime Environment installed, which can also be used to run OTP. Before proceding run `java -version` from the command prompt to check that you have a 1.8 version of the JVM installed (the specific sub-version doesn't matter). If you do not have the correct JRE installed you will need to install it for your operating system from: [http://www.oracle.com/technetwork/java/javase/downloads/jre8-downloads-2133155.html](http://www.oracle.com/technetwork/java/javase/downloads/jre8-downloads-2133155.html).*
