...
subtitle: "CENV6153 Transport Modelling - Transport Planning & GIS: Practical Session"
author: "Marcus Young"
...
This practical session uses version 1.5 of OTP and consists of three parts:
...
The datasets and applications used in this practical are available from a GitHub repository and can be downloaded in a single zip file called `materials-student-version.zip`, from here: [https://github.com/marcusyoung/otp-tutorial](https://github.com/marcusyoung/otp-tutorial).
...
OTP runs within a Java virtual machine (JVM), which on the University computers is provided by the Amazon Corretto distribution of the Open Java Development Kit (OpenJDK). We first need to confirm that the location of OpenJDK is configured as an operating system Environment Variable, enabling you to run the command `java` from the command prompt. Test this by entering `java -version` at the command prompt. If the version of OpenJDK is not returned, you will need to replace `java` with the following full path (including the quotation marks) when running the OTP commands to build a graph and start the OTP server:

```{r, engine = 'bash', eval = FALSE}
"C:\Program Files\Amazon Corretto\jdk11.0.13_8\bin\java.exe" 
```

![](images/hint.png) *If using your personal computer, you may have the Oracle Java Runtime Environment installed, which can also be used to run OTP. Before proceding run `java -version` from the command prompt to check that you have a 1.8 version of the JVM installed (the specific sub-version doesn't matter). If you do not have the correct JRE installed you will need to install it for your operating system from: [https://www.oracle.com/java/technologies/javase-jre8-downloads.html](https://www.oracle.com/java/technologies/javase-jre8-downloads.html).*

With the folder structure created and the files in place, we can now build the graph by running OTP and specifying the `--build` option. From the command prompt, change to the `otp` directory that you created and run the following command:
...
![](images/hint.png) *The GTFS schedules used in this tutorial are several months old; set the travel date to a day in January 2021 to see the full range of typical transit services.*
...
![](images/info.png) You can read more about the different debug layers in the official OTP documentation: [http://docs.opentripplanner.org/en/v1.5.0/Troubleshooting-Routing/#debug-layers](http://docs.opentripplanner.org/en/v1.5.0/Troubleshooting-Routing/#debug-layers).
...
[http://localhost:8080/otp/routers/current/isochrone?toPlace=53.3627432,-2.2729342&fromPlace=53.3627432,-2.2729342&arriveBy=TRUE&mode=WALK,TRANSIT&date=01-19-2021&time=08:00am&maxWalkDistance=1600&walkReluctance=5&minTransferTime=600&cutoffSec=900&cutoffSec=1800&cutoffSec=2700&cutoffSec=3600&cutoffSec=4500&cutoffSec=5400](http://localhost:8080/otp/routers/current/isochrone?toPlace=53.3627432,-2.2729342&fromPlace=53.3627432,-2.2729342&arriveBy=TRUE&mode=WALK,TRANSIT&date=01-19-2021&time=08:00am&maxWalkDistance=1600&walkReluctance=5&minTransferTime=600&cutoffSec=900&cutoffSec=1800&cutoffSec=2700&cutoffSec=3600&cutoffSec=4500&cutoffSec=5400)
...
gm_lsoa_centroids <- read.csv("materials-student-version/data/gm-lsoa-centroids.csv",
...
1 E01005756  391223   392954   53.43329  -2.13357     OK    56.52        12.5         1
2 E01005757  390660   391186   53.41739  -2.14199     OK    51.72        12.5         1
3 E01005754  390870   392662   53.43066  -2.13888     OK    61.45        12.5         1
4 E01005755  391140   391965   53.42440  -2.13479     OK    51.68        12.5         1
5 E01005752  393251   391229   53.41782  -2.10300     OK    66.83        13.5         1
6 E01005753  391339   392429   53.42858  -2.13181     OK    70.32        28.5         1
...
* **The official project documentation for OTP v1.5**: [http://docs.opentripplanner.org/en/v1.5.0/](http://docs.opentripplanner.org/en/v1.5.0/)
